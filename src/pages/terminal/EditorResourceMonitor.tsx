...
const EditorResourceMonitor = ({ uuid }: EditorResourceMonitorProps) => {
  ...
  return (
    <section className="shrink-0 border-t border-[#2b2b2b] bg-[#181818]" aria-label={t("file_manager.resource_monitor.title", "资源小窗")}>
      <div className="flex h-7 items-center gap-1.5 px-2 text-[10px] font-semibold uppercase text-neutral-500">
        <Gauge size={12} className="shrink-0" />
        <span>{t("file_manager.resource_monitor.title", "资源小窗")}</span>
        <img
      ...
      <div className="space-y-1.5 px-2 pb-2">
        {loadError && (
          <p className="text-[10px] text-red-300">
            {t("file_manager.resource_monitor.load_error", "实时数据加载失败")}
          </p>
        )}
        {!sample ? (
          <p className="pb-0.5 text-[10px] text-neutral-500">
            {t("file_manager.resource_monitor.waiting", "等待数据...")}
          </p>
        ) : !sample.online ? (
